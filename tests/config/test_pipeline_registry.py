...
def test_resolver_accepts_equivalent_config_class_by_model_type():
    class ExpectedConfig(PretrainedConfig):
        model_type = "same_model"

    class EquivalentConfig(PretrainedConfig):
        model_type = "same_model"

    @pipeline_cfg_resolver(config_type=ExpectedConfig)
    def resolver(hf_config):
        return PipelineConfig(model_type="resolved_type")

    assert resolver(EquivalentConfig()).model_type == "resolved_type"


def test_resolver_rejects_different_config_model_type():
    class ExpectedConfig(PretrainedConfig):
        model_type = "expected_model"

    class DifferentConfig(PretrainedConfig):
        model_type = "different_model"

    @pipeline_cfg_resolver(config_type=ExpectedConfig)
    def resolver(hf_config):
        return PipelineConfig(model_type="should_not_resolve")

    assert resolver(DifferentConfig()) is None


def test_resolver_can_explicitly_allow_missing_config():
    class ExpectedConfig(PretrainedConfig):
        model_type = "expected_model"

    @pipeline_cfg_resolver(config_type=ExpectedConfig, allow_none=True)
    def resolver(hf_config):
        return PipelineConfig(model_type="default_pipeline")

    assert resolver(None).model_type == "default_pipeline"
